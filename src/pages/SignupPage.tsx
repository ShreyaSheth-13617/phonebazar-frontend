    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md glass-card p-8">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          {form.role === "seller" ? "Join thousands of sellers on PhoneBazar" : "Join PhoneBazar today"}
        </p>
          ...
          {form.role !== "seller" && (
            <div className="flex gap-2">
              <button type="button" onClick={() => setForm({ ...form, role: "buyer" })}
                className={`flex-1 py-2 rounded-lg text-sm font-medium transition ${form.role === "buyer" ? "bg-primary text-primary-foreground" : "bg-secondary border border-border text-muted-foreground"}`}>
                Buyer
              </button>
              <button type="button" onClick={() => setForm({ ...form, role: "seller" })}
                className={`flex-1 py-2 rounded-lg text-sm font-medium transition ${form.role === "seller" ? "bg-primary text-primary-foreground" : "bg-secondary border border-border text-muted-foreground"}`}>
                Seller
              </button>
            </div>
          )}
          <button type="submit" className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold hover:opacity-90 transition">Create Account</button>
        </form>
